document.addEventListener("DOMContentLoaded", function() {
    ...
    function renderRecipes() {
        ...
        recipes.forEach(recipe => {
            ...
            recipeDiv.innerHTML = `
                ${recipe.image ? `<img src="${recipe.image}" alt="Recipe Image" style="max-width:120px;max-height:120px;">` : ''}
                <strong>${recipe.name}</strong> (${recipe.course})<br>
                <em>Ingredients:</em> ${recipe.ingredients.map(i => i.name + (i.quantity ? ' ('+i.quantity+')' : '')).join(', ')}<br>
    ...
    document.addEventListener('click', function(e) {
        ...
        if (e.target.classList.contains('edit-btn')) {
            ...
            if (recipe) {
                ...
                document.getElementById("ingredients").value = recipe.ingredients.map(i => i.name + (i.quantity ? ':'+i.quantity : '')).join(', ');
